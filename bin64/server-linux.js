...
app.use((req, res, next) => {
    // Always pass: static files, extension API, speedtest
    if (req.method === 'GET' && (req.path === '/' || /\.(html|js|css|ico|png|woff2?)$/.test(req.path)))
        return next();
    if (req.path.startsWith('/ext/') || req.path === '/speedtest')
        return next();
    // Node ping + speedtest bypass — used by extension without credentials
    if (/^\/nodes\/[^/]+\/(ping|speedtest)/.test(req.path))
        return next();

    // Dashboard auth toggle — if disabled, skip credential check
    const s = loadJson(SETTINGS_FILE, {});
    if (s.dashAuthEnabled === false) return next();
...
// Notification log — last 50 events sent to dashboard
const notifications = [];
...
// Active per-port 3proxy processes: { port: childProcess }
const portProxyProcesses = {};

// ─── PORT ROUTE CFG BUILDER ───────────────────────────────────────────────────
function buildPortRouteCfg(port, node) {
    const logPath = path.join(__dirname, `3proxy-port${port}.log`);
    // auth none + allow * so any client can connect without credentials
    return [
        'nserver 1.1.1.1',
        'nserver 8.8.8.8',
        'nscache 65536',
...
function startPortProxy(port, node) {
    if (portProxyProcesses[port]) {
        portProxyProcesses[port].kill('SIGTERM');
    ...
    // Make sure 3proxy binary is executable
    try { fs.chmodSync(PROXY_EXE, 0o755); } catch {}

    const proc = spawn(PROXY_EXE, [cfgPath], { detached: false, stdio: ['ignore','pipe','pipe'] });
    proc.stdout.on('data', d => console.log(`[port${port}] ${d.toString().trim()}`));
    proc.stderr.on('data', d => console.error(`[port${port}] ${d.toString().trim()}`));
    portProxyProcesses[port] = proc;
    proc.on('exit', (code) => {
        delete portProxyProcesses[port];
        if (code !== 0 && code !== null) {
            notify('warn', `Port ${port} proxy exited with code ${code}`);
        }
    });
    notify('info', `Port ${port} → ${node.name} (${node.tailscaleIp}) started`);
}
...
function stopPortProxy(port) {
    ...
    notify('info', `Port ${port} proxy stopped`);
}
...
function syncPortRoutes() {
    ...
    Object.entries(portRoutes).forEach(([port, nodeId]) => {
        ...
        if (node && node.enabled !== false && node.tailscaleIp) {
            ...
            stopPortProxy(port);
            notify('warn', `Port ${port}: node ${nodeId} not available — stopped`);
        }
    });
}
...
function startLogTailer() {
    if (!fs.existsSync(LOG_FILE)) { setTimeout(startLogTailer, 5000); return; }
    let size = fs.statSync(LOG_FILE).size;
    fs.watchFile(LOG_FILE, { interval: 500 }, () => {
        const newSize = fs.statSync(LOG_FILE).size;
        if (newSize > size) {
            const r2 = readline.createInterface({ input: fs.createReadStream(LOG_FILE, { start: size, end: newSize }) });
...
function parseLine(line) {
    ...
    const [, , clientIP, bytesIn, bytesOut, durationMs, sendRate, recvRate, remote] = parts;
...
function isOnline(lastSeen) { return lastSeen && (Date.now()-new Date(lastSeen).getTime())<180000; }
    ...
    try {
        ...
        if (status === 200 && body.devices) {
            ...
            tsDevices.forEach(dev => {
                ...
                if (node) { node.lastSeen=dev.lastSeen; node.online=isOnline(dev.lastSeen); node.hostname=dev.hostname||node.hostname; node.tailscaleIp=dev.addresses?.[0]||node.tailscaleIp; }
                const client = Object.values(clientRegistry).find(c=>c.tailscaleId===dev.id);
                if (client) { client.lastSeen=dev.lastSeen; client.online=isOnline(dev.lastSeen); client.hostname=dev.hostname||client.hostname; }
            });
...
function rewriteHubCfg() {
    if (!fs.existsSync(HUB_CFG_FILE)) {
        notify('error', 'Hub config file not found: ' + HUB_CFG_FILE);
        return false;
    }

    // Backup before writing
    fs.copyFileSync(HUB_CFG_FILE, HUB_CFG_FILE + '.bak');

    let cfg = fs.readFileSync(HUB_CFG_FILE, 'utf8');

    // Remove existing LB section
    cfg = cfg.replace(/# ── Load Balancer[\s\S]*?(?=# ──[^─]|$)/m, '');
    // Remove stray parent lines
    cfg = cfg.replace(/^allow \*\s*\nparent \d+ socks5 .+\n/gm, '');
    ...
    if (!hotloop.enabled || !primaryNode) {
        lbSection += '# Hotloop disabled — direct exit, no chaining\n';
        notify('info', 'Hotloop disabled — hub is exit node');
    } else {
        const pw = hotloop.primaryWeight;
        const fw = 1000 - pw;
        lbSection += `allow *\nparent ${pw} socks5 ${primaryNode.tailscaleIp} 1080\n`;
        notify('info', `Hotloop: primary → ${primaryNode.name} (${primaryNode.tailscaleIp}) weight=${pw}`);
        if (fallbackNode && fw > 0) {
            lbSection += `allow *\nparent ${fw} socks5 ${fallbackNode.tailscaleIp} 1080\n`;
            notify('info', `Hotloop: fallback → ${fallbackNode.name} (${fallbackNode.tailscaleIp}) weight=${fw}`);
        }
    }

    // Insert before first SOCKS5 directive, or append
    if (cfg.includes('# ── SOCKS5')) {
        cfg = cfg.replace(/(# ── SOCKS5)/, lbSection + '\n$1');
    } else if (cfg.includes('socks -p1080')) {
        cfg = cfg.replace(/(socks -p1080)/, lbSection + '\n$1');
    } else {
        cfg += lbSection;
    }

    fs.writeFileSync(HUB_CFG_FILE, cfg);
    notify('info', 'Hub config written successfully');
    return true;
}

// ─── PROXY CONTROL ────────────────────────────────────────────────────────────
function softRestartProxy(cb) {
    exec('systemctl is-active 3proxy-hub 2>/dev/null', (err, out) => {
        if (out && out.trim() === 'active') {
            exec('systemctl restart 3proxy-hub', e => {
                if (e) notify('error', 'Hub restart failed: ' + e.message);
                else   notify('info',  'Hub proxy restarted via systemctl');
                cb && cb(e);
            });
        } else {
            exec('pkill -f "3proxy.*3proxy.cfg" 2>/dev/null; true', () => {
                setTimeout(() => {
                    try { fs.chmodSync(PROXY_EXE, 0o755); } catch {}
                    const proc = spawn(PROXY_EXE, [HUB_CFG_FILE], { detached: true, stdio: 'ignore' });
                    proc.unref();
                    notify('info', 'Hub proxy restarted (standalone)');
                    cb && cb(null);
                }, 500);
            });
        }
...
// Toggle IP whitelist auth
app.post('/settings/auth', (req, res) => {
    settings.ipAuthEnabled = req.body.enabled;
    saveJson(SETTINGS_FILE, settings);
    syncWhitelist(getActualWhitelist());
    notify('info', `IP auth ${settings.ipAuthEnabled ? 'ENABLED' : 'DISABLED'}`);
    res.json({ ok: true, ipAuthEnabled: settings.ipAuthEnabled });
});

// Toggle dashboard password requirement
app.post('/settings/dash-auth', (req, res) => {
    settings.dashAuthEnabled = req.body.enabled;
    saveJson(SETTINGS_FILE, settings);
    notify('info', `Dashboard auth ${settings.dashAuthEnabled ? 'ENABLED' : 'DISABLED'}`);
    res.json({ ok: true, dashAuthEnabled: settings.dashAuthEnabled });
});

// Get notifications
app.get('/notifications', (req, res) => res.json({ notifications }));
app.delete('/notifications', (req, res) => { notifications.length = 0; res.json({ ok: true }); });

// ─── PORT ROUTES ──────────────────────────────────────────────────────────────
app.get('/port-routes', (req, res) => {
    const enriched = Object.entries(portRoutes).map(([port, nodeId]) => {
        const node = nodeRegistry[nodeId];
        return { port: parseInt(port), nodeId, nodeName: node?.name||'Unknown', nodeRegion: node?.region||'', active: !!portProxyProcesses[port] };
    });
...
app.post('/port-routes', (req, res) => {
    ...
    if (portNum === 1080 || portNum === 3128) return res.status(400).json({ error: 'Port 1080/3128 reserved' });
...
// Restart a specific port proxy
app.post('/port-routes/:port/restart', (req, res) => {
    const port = req.params.port;
    const nodeId = portRoutes[port];
    if (!nodeId) return res.status(404).json({ error: 'No route for this port' });
    const node = nodeRegistry[nodeId];
    if (!node?.tailscaleIp) return res.status(400).json({ error: 'Node has no Tailscale IP' });
    startPortProxy(port, node);
    res.json({ ok: true, msg: `Port ${port} → ${node.name} restarted` });
});
...
app.post('/restart', (req, res) => {
    const ok = rewriteHubCfg();
    if (!ok) return res.status(500).json({ error: 'Config write failed — check logs' });
    softRestartProxy(e => e ? res.status(500).json({ error: 'Restart failed', detail: e.message }) : res.json({ ok: true, msg: 'Hub restarted' }));
});
app.post('/start', (req, res) => {
    exec('systemctl is-active 3proxy-hub 2>/dev/null', (err, out) => {
        if (out && out.trim() === 'active') return res.json({ ok: true, msg: 'Already running' });
        exec('systemctl start 3proxy-hub', e => e ? res.status(500).json({ error: 'Failed' }) : res.json({ ok: true, msg: 'Started' }));
    });
});
app.post('/stop', (req, res) => {
    exec('systemctl stop 3proxy-hub 2>/dev/null; pkill -f "3proxy.*3proxy.cfg" 2>/dev/null; true', () => {
        notify('info', 'Hub proxy stopped');
        res.json({ ok: true, msg: 'Stopped' });
    });
});
app.post('/restart-web', (req, res) => { res.json({ ok: true, msg: 'Restarting web server…' }); setTimeout(()=>process.exit(0),500); });
app.post('/stop-web',    (req, res) => { res.json({ ok: true, msg: 'Stopping web server…'  }); setTimeout(()=>process.exit(0),300); });
app.get('/status', (req, res) => {
    exec('systemctl is-active 3proxy-hub 2>/dev/null', (err, out) => {
        const running = out && out.trim() === 'active';
        res.json({ running, portProxies: Object.keys(portProxyProcesses).length, portRouteCount: Object.keys(portRoutes).length });
    });
});
...
app.post('/config', (req, res) => { if (typeof req.body!=='string') return res.status(400).send('Expected text'); fs.writeFileSync(HUB_CFG_FILE+'.bak', fs.existsSync(HUB_CFG_FILE)?fs.readFileSync(HUB_CFG_FILE):''); fs.writeFileSync(HUB_CFG_FILE, req.body); notify('info','Hub config manually updated'); res.send('Saved'); });
...
app.get('/speedtest', (req, res) => {
    ...
    let sent=0; const chunk=Buffer.alloc(65536,0xAB);
...
app.post('/speedtest/upload', (req, res) => {
    ...
    req.on('end',()=>{ const e=(Date.now()-start)/1000; res.json({bytes,elapsed:e,mbps:((bytes*8)/e/1000000).toFixed(2)}); });
});
app.get('/ext/ping', (req, res) => { res.setHeader('Access-Control-Allow-Origin','*'); res.setHeader('Cache-Control','no-store'); res.json({ pong: true, ts: Date.now() }); });
...
app.post('/hotloop', (req, res) => {
    ...
    const cfgOk = rewriteHubCfg();
    if (!cfgOk) return res.status(500).json({ error: 'Failed to write hub config', notifications: notifications.slice(0,5) });

    setTimeout(() => softRestartProxy(() => {
        notify('info', `Hotloop updated — enabled=${hotloop.enabled} primary=${hotloop.primaryNode}`);
    }), 300);

    res.json({ ok: true, hotloop, msg: 'Config saved and hub restarting…', notifications: notifications.slice(0,5) });
});

// ─── NODES ────────────────────────────────────────────────────────────────────
function pingNode(ip, port, timeout) {
    return new Promise(resolve => {
        const start=Date.now(); const sock=new net.Socket();
...
app.get('/nodes', (req, res) => {
    const enriched = Object.entries(nodeRegistry).map(([id, node]) => {
        ...
        return { ...node, id, online: tsDev?isOnline(tsDev.lastSeen):node.online||false, lastSeen: tsDev?.lastSeen||node.lastSeen||null, assignedPort: assignedPort?parseInt(assignedPort):null, proxyRunning: assignedPort?!!portProxyProcesses[assignedPort]:null };
    });
...
// Restart 3proxy on a remote node via SSH over Tailscale
app.post('/nodes/:id/restart-proxy', (req, res) => {
    const node = nodeRegistry[req.params.id];
    if (!node) return res.status(404).json({ error: 'Node not found' });
    if (!node.tailscaleIp) return res.status(400).json({ error: 'Node has no Tailscale IP' });

    const cmd = `ssh -o StrictHostKeyChecking=no -o ConnectTimeout=10 root@${node.tailscaleIp} "systemctl restart 3proxy 2>/dev/null || service 3proxy restart 2>/dev/null || pkill 3proxy; sleep 1; 3proxy /etc/3proxy/conf/3proxy.cfg &" 2>&1`;
    exec(cmd, { timeout: 15000 }, (err, stdout, stderr) => {
        if (err) {
            notify('error', `Node ${node.name} 3proxy restart failed: ${err.message}`);
            return res.status(500).json({ error: 'SSH restart failed', detail: err.message, stderr });
        }
        notify('info', `Node ${node.name} (${node.tailscaleIp}) 3proxy restarted`);
        res.json({ ok: true, msg: `3proxy restarted on ${node.name}`, output: stdout });
    });
});
...
app.post('/nodes', (req, res) => {
    ...
    nodeRegistry[id] = { id, name, region: region||'Unknown', tailscaleIp: tailscaleIp||null, tailscaleId: tailscaleId||null, enabled: enabled!==false, offlineReason: offlineReason||null, maintenance: false, maintenanceReason: null, online: false, lastSeen: null, addedAt: new Date().toISOString(), connections: 0 };
    saveJson(NODES_FILE, nodeRegistry);
...
app.get('/nodes/join-instructions', (req, res) => {
    ...
    res.json({
        steps: [
            ...
            { label: 'Install 3proxy',      cmd: `sudo apt install 3proxy -y` },
            { label: 'Get Tailscale IP',    cmd: `tailscale ip -4` },
            { label: 'Enable & start',      cmd: `sudo systemctl enable 3proxy && sudo systemctl restart 3proxy` },
        ],
        note: 'Config: /etc/3proxy/conf/3proxy.cfg | Service: systemctl status 3proxy'
    });
});
...
app.patch('/clients/:id',(req,res)=>{const c=clientRegistry[req.params.id];if(!c)return res.status(404).json({error:'Not found'});if(req.body.defaultNode!==undefined)c.defaultNode=req.body.defaultNode;if(req.body.name)c.name=req.body.name;saveJson(CLIENTS_FILE,clientRegistry);res.json({ok:true,client:c});});
app.delete('/clients/:id',(req,res)=>{if(!clientRegistry[req.params.id])return res.status(404).json({error:'Not found'});delete clientRegistry[req.params.id];saveJson(CLIENTS_FILE,clientRegistry);res.json({ok:true});});
app.post('/clients/:id/kick', async (req, res) => {
    ...
    try{const{status}=await tsRequest('DELETE',`/device/${client.tailscaleId}`);if(status===200||status===204){client.online=false;saveJson(CLIENTS_FILE,clientRegistry);res.json({ok:true,message:`${client.name} kicked`});}else res.status(status).json({error:'Tailscale API error'});}
...
app.get('/stats', (req, res) => {
    ...
    const nodes = Object.entries(nodeRegistry).map(([id,n])=>({id,name:n.name,region:n.region,connections:n.connections||0,enabled:n.enabled,maintenance:n.maintenance||false,online:n.online}));
...
app.get('/ext/nodes', async (req, res) => {
    const allNodes = Object.values(nodeRegistry).map(n=>({
        id:n.id, name:n.name, region:n.region, tailscaleIp:n.tailscaleIp,
        enabled:n.enabled!==false, maintenance:n.maintenance||false,
        offlineReason:n.offlineReason||null, maintenanceReason:n.maintenanceReason||null,
    ...
    const portMap=Object.entries(portRoutes).reduce((acc,[p,nid])=>{acc[p]=nid;return acc;},{});
...
// ─── PER-NODE PING + SPEEDTEST (unauthenticated — used by extension) ──────────
app.get('/nodes/:id/ping', async (req, res) => {
    const node = nodeRegistry[req.params.id];
    if (!node?.tailscaleIp) return res.status(404).json({ error: 'Node not found or no IP' });
    const result = await pingNode(node.tailscaleIp, 1080, 3000);
    res.json({ pong: result.online, latencyMs: result.latencyMs, node: node.name, ts: Date.now() });
});
...
app.get('/nodes/:id/speedtest', (req, res) => {
    ...
    const proxyReq=http.request({hostname:node.tailscaleIp,port:3128,path:`http://${node.tailscaleIp}:3128/speedtest?size=${size}`,method:'GET',timeout:30000},proxyRes=>{res.setHeader('Content-Type','application/octet-stream');res.setHeader('Cache-Control','no-store');proxyRes.pipe(res);});
    proxyReq.on('error',()=>{const chunk=Buffer.alloc(65536,0xAB);let sent=0;res.setHeader('Content-Type','application/octet-stream');res.setHeader('Cache-Control','no-store');res.setHeader('X-Speedtest-Source','hub-fallback');function send(){while(sent<size){const n=Math.min(65536,size-sent);if(!res.write(chunk.slice(0,n))){res.once('drain',send);return;}sent+=n;}res.end();}send();});
    proxyReq.end();
});
