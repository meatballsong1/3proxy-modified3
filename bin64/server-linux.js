...
function generateRandomKey() {
    const words = ['alpha', 'beta', 'gamma', 'delta', 'echo', 'foxtrot', 'golf', 'hotel',
        ...
        'yankee', 'zulu', 'dog', 'cat', 'bird', 'fish', 'tree', 'rock', 'star', 'moon',
        'sun', 'cloud', 'rain', 'snow', 'wind', 'fire', 'water', 'earth', 'metal', 'wood'];
...
function isKeyValid(key) {
    const entry = Object.values(accessKeys).find(k => k.key === key);
...
app.get('/ext/nodes', async (req, res) => {
    ...
    if (!accessKey || !isKeyValid(accessKey)) {
...
app.delete('/keys/:id', (req, res) => {
    if (!accessKeys[req.params.id]) return res.status(404).json({ error: 'Key not found' });
    delete accessKeys[req.params.id];
    saveKeys();
    res.json({ ok: true });
});
...
app.post('/keys/validate', (req, res) => {
    ...
    if (!entry.enabled) return res.json({ valid: false, error: 'Key disabled' });
