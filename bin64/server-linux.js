...
const PROXY_EXE     = '/usr/bin/3proxy';
...
function startPortProxy(port, node) {
    ...
    const proc = require('child_process').spawn(PROXY_EXE, [cfgPath], { detached: false, stdio: 'ignore' });
...
function softRestartProxy(cb) {
    exec('systemctl is-active 3proxy-hub 2>/dev/null', (err, out) => {
        if (out && out.trim() === 'active') {
            exec('systemctl restart 3proxy-hub', err => cb && cb(err));
        } else {
            exec('pkill -f "3proxy.*3proxy.cfg" 2>/dev/null; true', () => {
                setTimeout(() => {
                    const proc = require('child_process').spawn(PROXY_EXE, [HUB_CFG_FILE], {
...
app.post('/start',       (req, res) => {
    exec('systemctl is-active 3proxy-hub 2>/dev/null', (err, out) => {
        if (out && out.trim() === 'active') return res.send('Already running');
        exec('systemctl start 3proxy-hub', e => e ? res.status(500).send('Failed') : res.send('Started'));
    });
});
...
app.post('/status',      (req, res) => {
    exec('systemctl is-active 3proxy-hub 2>/dev/null', (err, out) =>
        res.send(out && out.trim() === 'active' ? 'Running' : 'Not running'));
});
